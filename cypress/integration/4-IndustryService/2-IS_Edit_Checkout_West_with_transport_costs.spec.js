...
context('Industry Service Edit Environment',()=>{
    it('Checkout Bauabnahme 70 West with transportation costs for an unregistered private user',()=>{
        ...
        // Check that the initial price is 1260 euros
        cy.get('[class*="product-event-price-value"]').should('include.text', '1.260,00 €');
        ...
        cy.get('[id="ConstructionObject_PostalCode"]').type("86956");
        cy.get('[id="ConstructionObject_City"]').type("Schongau");
        cy.get('[id="ConstructionObject_Position"]').type("4.OG links");
        cy.get('[id="ConstructionObject_ObjectSize"]').type("67");
        ...
        cy.get('[name="Continue"]').click();
         ...
         // Check that the price did  change after entering the PLZ because it is in western Germany
         cy.get('[class="col-xs-12  list-item column-price"]').within(($columnPrice) =>{
            cy.get('strong').should('have.text','1.330,00 €')
        });

        // Click on "Objekdaten zeigen" to show all the data provided before
        cy.get('[class="text-link arrow-toggle collapsed"]').click()

        //Check that the data in the Objekdaten table is as provided before
        ...
        .parent('tr')
        .within(() => {            
        cy.get('td').eq(1).should('have.text','86956')
        })

        cy.contains('Stadt')
        .parent('tr')
        .within(() => {            
        cy.get('td').eq(1).should('have.text','Schongau')
        })
        ...
        //Check that Fahrtkosten were calculated correctly
        cy.get('[class="col-xs-3 col-sm-2 nopadding travelexpenses-value"]').should('have.text', '232,05 €')

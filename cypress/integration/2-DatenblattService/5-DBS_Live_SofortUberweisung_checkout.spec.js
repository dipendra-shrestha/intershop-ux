...
context('DBS Live Environment',()=>{
      it.skip('Rechnung Checkout clickpath',()=>{
          ...
          cy.get('[class="btn btn-primary pull-right"]').click({force:true});
          ...
          cy.get('[data-testing-id="payment-PAYONE SOFORT-Überweisung-element"]').check({force:true});

          //Clicking on 'Fortsetzung' button
          cy.get('[name="continue"]').click();
           ...
           //cy.forceVisit('https://www.sofort.com/payment/multipay/go/login');

          /*  cy.request({
            url: 'https://www.sofort.com/payment/multipay/go/login',
            followRedirect: true,
            timeout:30000})
           .then((response)=>{

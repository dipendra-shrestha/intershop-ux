...
context('Lift Service Edit Environment',()=>{
    it('Person lift Checkout 5 stops as unregistered company user',()=>{
        cy.visit(Cypress.env('preEditBaseUrl')+"/INTERSHOP/web/WFS/EasternEurope-AustriaLI-Site/de_AT/-/EUR/ViewHomepage-Start");
        ...
        cy.get('[id="ConstructionObject_PostalCode"]').type("1030");
        cy.get('[id="ConstructionObject_City"]').type("Wien");
        ...
        cy.get('[id="LiftInspectionExistingCertificates_PreviousAuditCompany"]').type("TÜV Rheinland");
        ...
        .parent('tr')
        .within(() => {            
        cy.get('td').eq(1).should('have.text','1030')
        })

        cy.contains('Stadt')
        .parent('tr')
        .within(() => {            
        cy.get('td').eq(1).should('have.text','Wien')
        })
